...
// Calcular total d'unitats en estoc
function totalUnitats() {
    return inventari.reduce((total, producte) => total + producte.stock, 0);
}

// Calcular total d'unitats venudes
function totalVenuts() {
    return inventari.reduce((total, producte) => total + producte.venuts, 0);
}

// Producte amb més estoc
function producteAmbMésEstoc() {
    return inventari.reduce((max, producte) => 
        producte.stock > max.stock ? producte : max
    );
}

// Producte amb menys estoc
function producteAmbMenyEstoc() {
    return inventari.reduce((min, producte) => 
        producte.stock < min.stock ? producte : min
    );
}
